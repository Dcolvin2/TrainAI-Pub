...
import { getCoreLift, muscleMap, dayStringMap } from "@/lib/coreMap";
...
export async function GET(req: NextRequest) {
  ...
  const trainingPattern = req.nextUrl.searchParams.get("trainingPattern");
  ...
  const coreLift = getCoreLift(weekday, trainingPattern || undefined);
  ...
  const prompt = buildClaudePrompt({
    ...
    duration: mins,
    equipment,
    trainingPattern: trainingPattern || undefined
  });
  ...
  try {
    ...
    if (trainingPattern) {
      plan.trainingPattern = trainingPattern;
    }
