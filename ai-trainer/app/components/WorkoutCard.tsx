...
export function WorkoutCard({ type, primaryMuscles, accentColor, onClick }: WorkoutCardProps) {
  return (
    <button
      onClick={onClick}
      className="relative overflow-hidden rounded-lg bg-card p-6 transition-all hover:scale-105 hover:bg-hover"
      style={{ borderTop: `3px solid ${accentColor}` }}
    >
      <h3 className="text-xl font-bold text-white mb-2">{type}</h3>
      <p className="text-sm text-secondary">{primaryMuscles}</p>
