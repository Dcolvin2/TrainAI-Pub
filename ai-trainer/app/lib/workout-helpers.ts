...
export function bodyAwareCooldown(msgLower: string): PlanItem[] {
  if (msgLower.includes("back") || msgLower.includes("pull") || msgLower.includes("lat")) {
    return [
      { name: "Cat-Cow", reps: "60s", sets: 1, restSeconds: 0, isAccessory: true },
      { name: "Child's Pose (Lat Focus)", reps: "60s", sets: 1, restSeconds: 0, isAccessory: true },
    ];
  }
  if (msgLower.includes("legs") || msgLower.includes("squat")) {
    return [
      { name: "Kneeling Hip Flexor Stretch", reps: "60s/side", sets: 1, restSeconds: 0, isAccessory: true },
      { name: "Seated Hamstring Stretch", reps: "60s/side", sets: 1, restSeconds: 0, isAccessory: true },
    ];
  }
  if (msgLower.includes("push") || msgLower.includes("chest") || msgLower.includes("shoulder")) {
    return [
      { name: "Doorway Pec Stretch", reps: "45s/side", sets: 1, restSeconds: 0, isAccessory: true },
      { name: "Sleeper Stretch", reps: "45s/side", sets: 1, restSeconds: 0, isAccessory: true },
    ];
  }
...
export function trimPlanToDuration(plan: WorkoutPlan, targetMinutes: number): WorkoutPlan {
  ...
  if (acc) {
    while (acc.items.length > 0 && currentEstimate() > targetMinutes) acc.items.pop();
  }
